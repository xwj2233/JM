...
class DownloadDb(object):
    ...
    def AddDownloadDB(self, task):
        ...
        sql = "INSERT INTO download(bookId, downloadEpsIds, curDownloadEpsId, curConvertEpsId, title, " \
            ...
            format(self.SqlText(task.bookId), json.dumps(task.epsIds), task.curDownloadEpsId, task.curConvertEpsId,
                   self.SqlText(task.title), self.SqlText(task.savePath), self.SqlText(task.convertPath),
                   task.status, task.convertStatus, tick)
    ...
    def AddDownloadEpsDB(self, info):
        ...
        sql = "INSERT INTO download_eps(bookId, epsId, epsTitle, picCnt, curPreDownloadIndex, curPreConvertId) " \
              ...
              "curPreConvertId = {5}".\
            format(self.SqlText(info.bookId), info.epsId, self.SqlText(info.epsTitle), info.picCnt, info.curPreDownloadIndex,
